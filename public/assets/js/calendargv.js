document.addEventListener('DOMContentLoaded', () => {
    ...
    calendarDates.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('calendar-date') && target.dataset.date) {
            ...
            const selectedStatus = document.getElementById('stat').value;
            const url = `/WebNav/public/includes/fetch_schedules.php?date=${encodeURIComponent(selectedDate)}&stat=${encodeURIComponent(selectedStatus)}`;
            console.log(`Request URL: ${url}`);
    ...
    const xhr = new XMLHttpRequest();
    const url = '/WebNav/public/includes/fetch_schedules.php';
